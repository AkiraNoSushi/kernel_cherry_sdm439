...
int malloc_struct_DetailThreshold(void)
{
	...
	if (NULL == test_data.mcap_detail_thr.panel_differ_test_min)
		goto ERR;
	test_data.mcap_detail_thr.PanelDiffer_Uniformity_TxLinearityTest_Max = (int (*)[RX_NUM_MAX])fts_malloc(NUM_MAX * sizeof(int));
	if (NULL == test_data.mcap_detail_thr.PanelDiffer_Uniformity_TxLinearityTest_Max)
		goto ERR;
	test_data.mcap_detail_thr.PanelDiffer_Uniformity_RxLinearityTest_Max = (int (*)[RX_NUM_MAX])fts_malloc(NUM_MAX * sizeof(int));
	if (NULL == test_data.mcap_detail_thr.PanelDiffer_Uniformity_RxLinearityTest_Max)
		goto ERR;
...
void free_struct_DetailThreshold(void)
{
	...
	if (NULL != test_data.mcap_detail_thr.PanelDiffer_Uniformity_TxLinearityTest_Max) {
		fts_free(test_data.mcap_detail_thr.PanelDiffer_Uniformity_TxLinearityTest_Max);
		test_data.mcap_detail_thr.PanelDiffer_Uniformity_TxLinearityTest_Max = NULL;
	}
	if (NULL != test_data.mcap_detail_thr.PanelDiffer_Uniformity_RxLinearityTest_Max) {
		fts_free(test_data.mcap_detail_thr.PanelDiffer_Uniformity_RxLinearityTest_Max);
		test_data.mcap_detail_thr.PanelDiffer_Uniformity_RxLinearityTest_Max = NULL;
	}
...
void OnInit_DThreshold_PanelDiffer_Uniformity_Test(char *strIniFile)
{
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		for (j = 0; j < RX_NUM_MAX; j++) {
			test_data.mcap_detail_thr.PanelDiffer_Uniformity_TxLinearityTest_Max[i][j] = 0;
		}
	}
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.PanelDiffer_Uniformity_TxLinearityTest_Max[i][k] = (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
			} else {
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		for (j = 0; j < RX_NUM_MAX; j++) {
			test_data.mcap_detail_thr.PanelDiffer_Uniformity_RxLinearityTest_Max[i][j] = 0;
		}
	}
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				test_data.mcap_detail_thr.PanelDiffer_Uniformity_RxLinearityTest_Max[i][k] = (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
			} else {
